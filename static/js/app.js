...
/* Identity is now server-side. The session is the source of truth; this
   object is only a local mirror of it, refreshed from /api/auth/me on boot.
   Editing it in devtools changes nothing — every write endpoint re-checks the
   session, so the server would return 403. (KNOWN_GAPS B-1, closed.) */
let ME = null;
const state = {
  roleKey: "physician",   // mirror of the session; set by boot()
  scale: [], counts: {}, collapsed: localStorage.getItem("ed_collapsed") === "1",
  boardTimer: null,
};
/* The server derives the audit actor from the session. We never send it. */
function actor() { return ME ? `${ME.name} (${ME.role})` : "system"; }
function can(a) { return !!ME && ME.perms.includes(a); }
function roleName() { return ME ? ME.role : ""; }
  ...
  async req(method, path, body) {
    try {
      ...
      if (r.status === 401 && !path.startsWith("/api/auth/")) {
        location.href = "/login";                 // session gone — re-auth
        return { ok:false, status:401, data:null };
      }
      if (r.status === 403 && data) {             // server refused the role
        toast("err", "Not permitted", data.message || "Your role cannot do that.");
      }
...
function openRoleMenu() {
  ...
  root.innerHTML = `<div class="scrim" style="background:transparent;backdrop-filter:none">
    <div class="menu" style="position:fixed;left:${rect.left}px;bottom:${window.innerHeight-rect.top+8}px" role="menu">
      <div class="p-sec">Signed in</div>
      <div class="mi on" style="cursor:default">
        <span class="av">${esc(ME?ME.init:"?")}</span>
        <div><div style="font-weight:600">${esc(ME?ME.name:"—")}</div>
          <div class="muted" style="font-size:11px">${esc(ME?ME.role:"—")}</div>
          <div class="muted mono" style="font-size:10px">${esc(ME?ME.email:"")}</div></div>
      </div>
      <div class="sep"></div>
      ${can("reset_demo")?`<div class="mi" data-reset="1" role="menuitem">${icon("db")}<span>Reset demo data</span></div>`:""}
      <div class="mi" data-logout="1" role="menuitem" style="color:var(--coral-2)">${icon("x")}<span>Sign out</span></div>
    </div></div>`;
  const scrim = $(".scrim", root);
  scrim.addEventListener("mousedown", e => { if (e.target === scrim) closeOverlay(); });
  document.addEventListener("keydown", overlayKey);
  $(".mi[data-logout]", root).onclick = async () => {
    closeOverlay();
    await api.post("/api/auth/logout");
    location.href = "/login";
  };
  const resetMi = $(".mi[data-reset]", root);
  if (resetMi) resetMi.onclick = async () => {
    closeOverlay();
...
/* ---- Dashboard — ED situational awareness ----
   Redesigned for the 3-5 second read. A doctor walking up to this screen must
   learn, in order: is anyone dying, who is next, how bad is the load.
   Aggregates alone cannot answer "who do I see next" — so the queue is the
   dashboard, and the KPIs sit above it as context.
   Backend unchanged: /api/dashboard + /api/board already return everything. */

const DASH = { q: "", filter: "all", timer: null };

function waitClass(row) {
  if (!row.target_minutes && row.target_minutes !== 0) return "";
  const e = row.elapsed_minutes || 0, t = row.target_minutes;
  if (t === 0) return e > 0 ? "w-over" : "";          // L1: target is immediate
  if (e > t) return "w-over";
  if (e >= t * 0.75) return "w-near";
  return "";
}
function statusPill(s) {
  const m = { ARRIVED:["Awaiting triage","st-arrived"], TRIAGED:["Awaiting physician","st-triaged"],
              IN_TREATMENT:["In treatment","st-treat"], CLOSED:["Closed","st-closed"] };
  const [t, c] = m[s] || [pretty(s), ""];
  return `<span class="st ${c}">${esc(t)}</span>`;
}
/* Patient journey: arrival -> triage -> physician -> disposition */
function journey(row) {
  const steps = [
    { k:"Arrived",   done: true },
    { k:"Triaged",   done: !!row.triaged_ts },
    { k:"Physician", done: !!row.first_physician_at },
    { k:"Disposed",  done: row.status === "CLOSED" },
  ];
  return `<span class="jrn" title="${steps.map(s=>s.k+(s.done?" ✓":" ·")).join("  ")}">${
    steps.map(s=>`<i class="${s.done?'on':''}"></i>`).join("")}</span>`;
}
...
async function viewDashboard() {
  ...
  active.sort((a, b) =>
    ((a.level ?? 9) - (b.level ?? 9)) || ((b.elapsed_minutes||0) - (a.elapsed_minutes||0)));

  const critical  = active.filter(r => r.level != null && r.level <= 2);
  const untriaged = active.filter(r => r.status === "ARRIVED");
  const waiting   = active.filter(r => r.status !== "IN_TREATMENT");
  const treating  = active.filter(r => r.status === "IN_TREATMENT");
  ...
  const loadScore = Math.min(100, Math.round(
    (active.length * 6) + (critical.length * 18) + (breached.length * 10)));
  const loadTone  = loadScore >= 70 ? "crit" : loadScore >= 40 ? "warn" : "ok";
  const loadWord  = loadScore >= 70 ? "HEAVY" : loadScore >= 40 ? "MODERATE" : "NORMAL";

  const next = active.find(r => r.status !== "IN_TREATMENT");

  // ---- filter + search over the queue (view-only; never mutates the data) ----
  const f = DASH.filter, q = DASH.q.trim().toLowerCase();
  let shown = active.filter(r => {
    if (f === "critical" && !(r.level != null && r.level <= 2)) return false;
    if (f === "waiting"  && r.status === "IN_TREATMENT") return false;
    if (f === "treating" && r.status !== "IN_TREATMENT") return false;
    if (f === "breach"   && !r.is_breached) return false;
    if (f === "mlc"      && !r.is_mlc) return false;
    if (!q) return true;
    return [r.display_name, r.identifier, r.chief_complaint, r.mlc_serial]
      .some(x => String(x||"").toLowerCase().includes(q));
  });

  const clinical = can("triage") || can("dispose") || can("attend");

  const kpis = [
    { label:"Waiting",       val:waiting.length,  foot:`${untriaged.length} not yet triaged`,
      tone: untriaged.length ? "warn" : "", ic:"users", filter:"waiting" },
    { label:"Critical",      val:critical.length, foot:"Level 1–2 · see immediately",
  ...
  const chip = (k, lbl, n) =>
    `<button class="chip ${DASH.filter===k?'on':''}" data-f="${k}">${esc(lbl)}<b>${n}</b></button>`;

  v.innerHTML = `<div class="view-inner view-anim dash">

    <div class="page-head">
      <div class="h">
        <h1>Emergency Department</h1>
        <div class="sub">${active.length} active · updated <span class="mono" id="dashTime">${esc(fmtTs(d.generated_at))}</span>
    ...
    ${(clinical && critical.length) ? `<div class="crit-strip">
      <span class="cs-ic">${icon("alert")}</span>
      <span class="cs-t"><b>${critical.length}</b> patient${critical.length===1?"":"s"} require immediate attention</span>
    ...
    <div class="dash-kpis">
      ${kpis.map(k=>`
        <button class="dk ${k.tone} ${DASH.filter===k.filter?'on':''}" data-f="${k.filter}">
          <div class="dk-top"><span class="ic">${icon(k.ic)}</span>${esc(k.label)}</div>
          <div class="dk-val">${k.val}${k.unit?`<small>${k.unit}</small>`:""}</div>
          <div class="dk-foot">${esc(k.foot)}</div>
        </button>`).join("")}

      <div class="dk load ${loadTone}">
        <div class="dk-top"><span class="ic">${icon("activity")}</span>Department load</div>
        <div class="dk-val">${loadWord}</div>
    ...
    ${(clinical && next) ? `<div class="next-up">
      <div class="nu-tag">TREAT NEXT</div>
      <span class="lvl-badge ${levelClass(next.level)}">${next.level ?? "—"}</span>
      <div class="nu-who">
        <div class="nu-name">${esc(next.display_name)} ${next.is_mlc?`<span class="mlc-tag">MLC</span>`:""}</div>
        <div class="nu-sub">${esc(next.chief_complaint || "Not yet triaged")}</div>
      </div>
      <div class="nu-wait ${waitClass(next)}">
        <div class="nu-min">${next.elapsed_minutes ?? 0}<small>min</small></div>
        <div class="nu-tgt">${next.target_minutes===0?"immediate":next.target_minutes!=null?`target ${next.target_minutes}m`:"—"}</div>
      </div>
      <a class="btn primary" href="#/encounter/${next.encounter_id}">Open ${icon("chevronR")}</a>
    </div>` : ""}

    <div class="card q-card">
      <div class="q-hd">
        <div class="q-title"><span class="card-title-icn">${icon("board")}</span>
          <h3>Live queue</h3><span class="sub">by acuity, then longest wait</span></div>
        <div class="q-tools">
          <div class="q-search">${icon("search")}<input id="qSearch" placeholder="Search name, ID, complaint…" value="${esc(DASH.q)}" autocomplete="off"></div>
        </div>
      </div>
      <div class="q-chips">
        ${chip("all","All",active.length)}
        ${chip("critical","Critical",critical.length)}
        ${chip("waiting","Waiting",waiting.length)}
        ${chip("treating","In treatment",treating.length)}
        ${chip("breach","Breached",breached.length)}
        ${chip("mlc","MLC",active.filter(r=>r.is_mlc).length)}
      </div>

      ${shown.length ? `<ol class="queue">
        ${shown.map((r,i) => `
          <li class="qr ${r.level!=null&&r.level<=2?'is-crit':''} ${r.is_breached?'is-breach':''} ${r.status==='IN_TREATMENT'?'is-treat':''}">
            <span class="q-pos">${r.status==='IN_TREATMENT' ? '<span class="q-dot"></span>' : i+1}</span>
            <span class="lvl-badge ${levelClass(r.level)}">${r.level ?? "—"}</span>
            <span class="q-who">
              <span class="q-name">${esc(r.display_name)}
                ${r.is_unknown?`<span class="unk-tag">UNIDENTIFIED</span>`:""}
                ${r.is_mlc?`<span class="mlc-tag">${esc(r.mlc_serial||"MLC")}</span>`:""}</span>
              <span class="q-cc">${esc(r.chief_complaint || "Awaiting triage")}</span>
            </span>
            <span class="q-jrn">${journey(r)}</span>
            <span class="q-status">${statusPill(r.status)}</span>
            <span class="q-wait ${waitClass(r)}">
              <b>${r.elapsed_minutes ?? 0}</b><small>min</small>
              ${r.is_breached?`<span class="ovr">OVER</span>`:""}
            </span>
            <a class="q-go" href="#/encounter/${r.encounter_id}" title="Open encounter">${icon("chevronR")}</a>
          </li>`).join("")}
      </ol>` : emptyInline(q||f!=="all" ? "No patients match this filter" : "Department is clear — no active patients")}
    </div>

    ${(can("mlc") && d.intimation_pending) ? `<div class="stat-note">${icon("gavel")}
      <span><b>${d.intimation_pending}</b> MLC case${d.intimation_pending===1?"":"s"} with no police intimation logged.
      Disposition will warn (BNSS 2023 §194–196) until recorded or justified.</span>
      <a class="btn sm" href="#/mlc">Open MLC register</a></div>` : ""}

  </div>`;

  // ---- interactions ----
  $$(".dk[data-f]", v).forEach(b => b.onclick = () => {
    DASH.filter = (DASH.filter === b.dataset.f) ? "all" : b.dataset.f; viewDashboard(); });
  $$(".chip[data-f]", v).forEach(b => b.onclick = () => { DASH.filter = b.dataset.f; viewDashboard(); });

  const s = $("#qSearch", v);
  if (s) {
    s.oninput = () => { DASH.q = s.value; clearTimeout(DASH.deb);
      DASH.deb = setTimeout(() => { const p = s.selectionStart; viewDashboard();
        const n = $("#qSearch"); if (n) { n.focus(); n.setSelectionRange(p, p); } }, 220); };
  }

  // Auto-refresh, 15s. Same teardown contract as the board (router clears it).
  clearTimeout(state.boardTimer);
  state.boardTimer = setTimeout(() => {
    const h = location.hash.replace(/^#/, "");
    if (h === "" || h === "/") viewDashboard();
  }, 15000);
}
...
function viewRegister() {
  ...
  v.innerHTML = `<div class="view-inner view-anim reg">
    <header class="reg-head">
      <h1>Quick registration</h1>
      <p class="reg-sub">Treat-first · nothing on this screen blocks care — Art. 21 / Parmanand Katara (SC 1989).</p>
    </header>

    <section class="reg-emergency">
      <div class="re-l">
        <div class="re-title">${icon("siren")} UNKNOWN / UNCONSCIOUS PATIENT</div>
        <div class="re-desc">One click issues a temporary ID and a triage slot. Paperwork reconciles later — or never.</div>
      </div>
      <button class="btn coral re-btn" id="qUnknown">${icon("zap")} Register now</button>
    </section>

    <form id="regForm" class="reg-form">
      <div class="reg-form-head"><span class="rfh-l">Identified patient</span><span class="rfh-r">ALL FIELDS OPTIONAL</span></div>
      <div class="reg-grid">
        <div class="field f-name"><label>Full name</label><input class="inp" name="name" autocomplete="off" placeholder="e.g. Ramesh Kumar"></div>
        <div class="field f-age"><label>Age</label><input class="inp" name="age_years" type="number" min="0" max="130" inputmode="numeric"></div>
      ...
      <div class="reg-actions">
        <button type="submit" class="btn primary">${icon("check")} Register & go to triage</button>
        <a class="btn ghost reg-cancel" href="#/board">Cancel</a>
      </div>
      <div class="reg-info">${icon("info")}<span>Suggested triage level is computed on the next screen — advisory only; the nurse confirms.</span></div>
    </form>
  </div>`;
      ...
      <div style="display:flex;flex-direction:column;gap:16px;position:sticky;top:0">
        ...
        <div class="card"><div class="hd"><span class="card-title-icn">${icon("pulse")}</span><h3>Confirm final level</h3></div>
          <div class="bd">
            ...
            ${can("triage")?"":`<div class="hint mt3">${icon("info")} Your role (${esc(roleName())}) is not permitted to confirm triage. A nurse or physician must do this.</div>`}
          </div></div>
      </div>
    </div></div>`;
...
async function viewEncounter(id) {
  ...
  v.innerHTML = `<div class="view-inner view-anim enc">
    <header class="enc-head">
      <div class="enc-top">ENC-${id} · <span class="enc-status">${statusText(e.status)}</span></div>
      <div class="enc-main">
        <h1 class="enc-name">${esc(name)}${e.is_mlc?`<span class="badge mlc enc-mlctag">${icon("gavel")} MLC</span>`:""}</h1>
        <div class="enc-actions">${actions.join("")||noActions}</div>
      </div>
      <div class="enc-meta">${esc(p?(p.uhid||p.temp_id):"—")} · arrived ${fmtTs(e.arrival_ts)} · ${pretty(e.arrival_mode||"—")}</div>
    </header>

    ${D.intimation_pending?`<div class="enc-warn">${icon("alert")}<div><div class="wt">MLC intimation pending</div>
    ...
    <div class="enc-grid">
      <section class="enc-sec">
        <div class="enc-h"><span>Patient information</span></div>
        <dl class="enc-dl">
      ...
      <section class="enc-sec">
        <div class="enc-h"><span>Triage history</span><span class="enc-h-sub">${D.triages.length} assessment(s)</span></div>
        ${D.triages.length?`<ul class="timeline">${D.triages.map(triageItem).join("")}</ul>`:emptyInline("Not yet triaged")}
      </section>
    </div>
...
function encCol(label, value, cls) {
  return `<div class="ms-col"><span class="ms-l">${esc(label)}</span><span class="ms-v ${cls||""}">${value}</span></div>`;
}
function mlcStrip(m, intims, encId) {
  const intim = (intims && intims.length)
    ? `<span style="color:var(--teal-ink)">${intims.length} logged</span>`
    : `<span style="color:var(--coral-2)">none — pending</span>`;
  return `<section class="enc-sec enc-strip">
    <div class="enc-h"><span>Medico-legal case</span>${m.pocso_flag?`<span class="badge pocso">POCSO</span>`:""}</div>
    <div class="mlc-row">
      ${encCol("Serial", esc(m.mlc_serial), "coral")}
      ${encCol("Type", pretty(m.mlc_type))}
      ${encCol("Opened", fmtTs(m.opened_ts), "mono")}
      ${encCol("By", esc(m.opened_by))}
      ${encCol("Intimations", intim)}
      <a class="btn sm" href="#/mlc/${m.id}">Open ${icon("chevronR")}</a>
    </div>
  </section>`;
}
function dispositionStrip(d) {
  const extra = d.ward_requested ? "Ward " + esc(d.ward_requested)
    : d.referral_facility ? esc(d.referral_facility) + (d.referral_reason?" — "+esc(d.referral_reason):"")
    : d.discharge_instr ? esc(d.discharge_instr)
    : d.cause_of_death_icd10 ? "ICD-10 " + esc(d.cause_of_death_icd10) : "—";
  return `<section class="enc-sec enc-strip">
    <div class="enc-h"><span>Disposition</span><span class="badge status">${pretty(d.type)}</span></div>
    <div class="mlc-row">
      ${encCol("Type", pretty(d.type))}
      ${encCol("Decided", fmtTs(d.decided_ts), "mono")}
      ${encCol("By", esc(d.decided_by))}
      <div class="ms-col ms-wide"><span class="ms-l">Detail</span><span class="ms-v">${extra}</span></div>
    </div>
    ${d.mlc_warning_ack?`<div class="stat" style="color:var(--coral-2);font-size:12px;margin-top:8px">${icon("info")} MLC proceed-justification: ${esc(d.mlc_warning_reason||"acknowledged")}</div>`:""}
  </section>`;
}
...
async function init() {
  ...
  if (!me.ok || !me.data || !me.data.user) { location.href = "/login"; return; }
  ME = me.data.user;
  state.roleKey = ME.key;
